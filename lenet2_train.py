...
# Inside each layer, capture and accumulate data as needed
def capture_layer_data(layer_name, activations, weights, biases):
...
layer_data = {}
    ...
    for i in range(EPOCHS):
        ...
        for offset in range(0, num_examples, BATCH_SIZE):
            ...
            # Capture data for Layer 1 after each forward pass
            activations1, weights1, biases1 = sess.run([conv1, conv1_W, conv1_b], feed_dict={x: batch_x})
            capture_layer_data('conv1', activations1, weights1, biases1)
            # Capture data for Layer 2 after each forward pass
            activations2, weights2, biases2 = sess.run([conv2, conv2_W, conv2_b], feed_dict={x: batch_x})
            capture_layer_data('conv2', activations2, weights2, biases2)
            # Capture data for Layer 3 after each forward pass
            activations3, weights3, biases3 = sess.run([fc1, fc1_W, fc1_b], feed_dict={x: batch_x})
            capture_layer_data('fc1', activations3, weights3, biases3)
            # Capture data for Layer 4 after each forward pass
            activations4, weights4, biases4 = sess.run([fc2, fc2_W, fc2_b], feed_dict={x: batch_x})
            capture_layer_data('fc2', activations4, weights4, biases4)
            # Capture data for Layer 5 (Output Layer) after each forward pass
            activations5, weights5, biases5 = sess.run([logits, fc3_W, fc3_b], feed_dict={x: batch_x})
            capture_layer_data('logits', activations5, weights5, biases5)
