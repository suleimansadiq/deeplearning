...
**Layer 5: Fully Connected (Logits).** Input = 84. Output = 10.
...
def LeNet(x):
    ...
    # Save activations of Layer 1
    with tf.variable_scope('layer1_activations'):
        activations_layer1 = tf.identity(conv1, name='activations_layer1')
    ...
    # Save activations of Layer 2
    with tf.variable_scope('layer2_activations'):
        activations_layer2 = tf.identity(conv2, name='activations_layer2')
    ...
    # Save activations of Layer 3
    with tf.variable_scope('layer3_activations'):
        activations_layer3 = tf.identity(fc1, name='activations_layer3')
    ...
    # Save activations of Layer 4
    with tf.variable_scope('layer4_activations'):
        activations_layer4 = tf.identity(fc2, name='activations_layer4')
    ...
    with tf.variable_scope('logits'):
        logits_tensor = tf.identity(logits, name='logits')
...
def evaluate(X_data, y_data):
    ...
    for offset in range(0, num_examples, BATCH_SIZE):
        ...
        accuracy, top5 = sess.run(
            [accuracy_operation, top5_operation], feed_dict={x: batch_x, y: batch_y})
...
# Train LeNet on the given dataset and store the batch activations
with tf.Session() as sess:
    ...
    for i in range(EPOCHS):
        ...
        for offset in range(0, num_examples, BATCH_SIZE):
            ...
            activations_layer1_val, activations_layer2_val, activations_layer3_val, activations_layer4_val = sess.run(
                [activations_layer1, activations_layer2, activations_layer3, activations_layer4],
                feed_dict={x: batch_x, y: batch_y}
            )
            batch_index = offset // BATCH_SIZE
            save_activations(activations_layer1_val, 'layer1', batch_index)
            save_activations(activations_layer2_val, 'layer2', batch_index)
            save_activations(activations_layer3_val, 'layer3', batch_index)
            save_activations(activations_layer4_val, 'layer4', batch_index)
            activations_dict['layer1'] = activations_layer1_val
            activations_dict['layer2'] = activations_layer2_val
            activations_dict['layer3'] = activations_layer3_val
            activations_dict['layer4'] = activations_layer4_val
            batch_index = offset // BATCH_SIZE
            save_activations(activations_layer1_val, 'layer1', batch_index)
            save_activations(activations_layer2_val, 'layer2', batch_index)
            save_activations(activations_layer3_val, 'layer3', batch_index)
            save_activations(activations_layer4_val, 'layer4', batch_index)
        validation_accuracy, validation_top5 = evaluate(X_test, y_test)
        print("EPOCH {} ...".format(i+1))
        print("Validation Accuracy = {:.3f}".format(validation_accuracy))
        print("Validation Top-5 Accuracy = {:.3f}".format(validation_top5))
        print()
    save_layer_data('activations', activations_dict)
