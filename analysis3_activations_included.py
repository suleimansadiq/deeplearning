...
# Define a custom data type for tf.posit8
tf_type = tf.posit8
...
def main():
    ...
    with tf.Session() as sess:
        ...
        # Export activations
        x = tf.placeholder(tf_type, (None, 32, 32, 1))
        logits = LeNet(x)
        activations_to_export = {
            'conv1': tf.nn.relu(tf.nn.conv2d(x, conv1_W, strides=[1, 1, 1, 1], padding='VALID') + conv1_b),
            'conv2': tf.nn.relu(tf.nn.conv2d(tf.nn.max_pool(activations_to_export['conv1'], ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='VALID'), conv2_W, strides=[1, 1, 1, 1], padding='VALID') + conv2_b),
            'fc1': tf.nn.relu(tf.matmul(fc0, fc1_W) + fc1_b),
            'fc2': tf.nn.relu(tf.matmul(activations_to_export['fc1'], fc2_W) + fc2_b),
            'fc3': tf.matmul(activations_to_export['fc2'], fc3_W) + fc3_b,
        }

        for activation_name, activation_tensor in activations_to_export.items():
            activations = sess.run(activation_tensor, feed_dict={x: your_input_data})
            export_to_csv(activation_name, activations)
