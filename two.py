...
with tf.Session() as sess:
            ...
            for n in range(num_filters):
                for i in range(input_shape[0] - filter_height + 1):
                    for j in range(input_shape[1] - filter_width + 1):
                        ...
                        constraints.append(f"Weighted sum for y_{i}_{j}_{n} in {layer_name}: {linear_sum} >= 0")
                        constraints.append(f"ReLU activation for y_{i}_{j}_{n} in {layer_name}: y_{i}_{j}_{n} = if {linear_sum} >= 0 then {linear_sum} else 0")
                        node_count += 1
        else:  # Fully connected layer
            for i in range(len(biases)):
                linear_sum = f"{biases[i]}" + " + " + " + ".join([f"{weights[j, i]}*x_{j}" for j in range(weights.shape[0])])
                constraints.append(f"Weighted sum for y_{i} in {layer_name}: {linear_sum} >= 0")
                constraints.append(f"ReLU activation for y_{i} in {layer_name}: y_{i} = if {linear_sum} >= 0 then {linear_sum} else 0")
                node_count += 1
        ...
        for i in range(shape[0]):
            for j in range(shape[1]):
                for k in range(shape[2]):
                    constraints.append(f"Max pooling constraint for y_{i}_{j}_{k} in {name}: y_{i}_{j}_{k} >= 0")
                    constraints.append(f"ReLU activation for y_{i}_{j}_{k} in {name}: y_{i}_{j}_{k} = if y_{i}_{j}_{k} >= 0 then y_{i}_{j}_{k} else 0")
                    total_nodes += 1
                    total_constraints += 2
    ...
    for i in range(flatten_shape):
        constraints.append(f"Flatten constraint for y_{i} in flatten: y_{i} >= 0")
        constraints.append(f"ReLU activation for y_{i} in flatten: y_{i} = if y_{i} >= 0 then y_{i} else 0")
        total_nodes += 1
        total_constraints += 2
